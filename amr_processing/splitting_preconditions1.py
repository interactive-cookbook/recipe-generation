...
def check_split_condition(amr_graph: nx.Graph, node1, node2) -> bool:
    ...
    for ind, path in enumerate(labelled_path_triples):
        ...
        path_nodes = [triple[0] for triple in path]         # TODO: which version is correct??
        ...
        if path_edges[0] in edges_of_interest_forward:
            to_split = check_path_of_interest(path_edges=path_edges, path_nodes=path_nodes,
                                              path_nodes_unlabelled=unlabelled_path_nodes, forward=True, graph=amr_graph)
            if not to_split:
                break

        if path_edges[-1] in edges_of_interest_backward:
            to_split = check_path_of_interest(path_edges=path_edges, path_nodes=path_nodes,
                                              path_nodes_unlabelled=unlabelled_path_nodes, forward=False, graph=amr_graph)
            if not to_split:
                break
...
def check_path_of_interest(path_edges, path_nodes, path_nodes_unlabelled, forward, graph) -> bool:
    """
